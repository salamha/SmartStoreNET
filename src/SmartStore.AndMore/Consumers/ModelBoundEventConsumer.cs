...
namespace SmartStore.AndMore
{
    public class ModelBoundEventConsumer : IConsumer
    {
        ...
        {
            if (!eventMessage.BoundModel.CustomProperties.ContainsKey("SmartStoreAndMore"))
                return;

            var model = eventMessage.BoundModel.CustomProperties["SmartStoreAndMore"] as AdminEditTabModel;
            if (model == null)
                return;

            var settings = _settingService.LoadSetting<SmartStoreAndMoreSettings>(_storeContext.CurrentStore.Id);
